...
// GANTI ENTRY IDs INI SESUAI DENGAN FIELD DI GOOGLE FORM ANDA
// Cara mendapatkan entry ID: inspect element pada form, cari name="entry.xxxxx"
const FORM_FIELDS = {
  orderNumber: "entry.1234567890", // Ganti dengan entry ID untuk Nomor Pesanan
  nama: "entry.2345678901", // Ganti dengan entry ID untuk Nama
  email: "entry.3456789012", // Ganti dengan entry ID untuk Email
  noHp: "entry.4567890123", // Ganti dengan entry ID untuk No HP
  paket: "entry.5678901234", // Ganti dengan entry ID untuk Paket
  harga: "entry.6789012345", // Ganti dengan entry ID untuk Harga
  jenisDesain: "entry.7890123456", // Ganti dengan entry ID untuk Jenis Desain
  deskripsi: "entry.8901234567", // Ganti dengan entry ID untuk Deskripsi
  deadline: "entry.9012345678", // Ganti dengan entry ID untuk Deadline
  metode: "entry.0123456789", // Ganti dengan entry ID untuk Metode Pembayaran
  buktiPembayaran: "entry.1234567891", // Ganti dengan entry ID untuk Bukti Pembayaran
};
...
async function submitToGoogleForm(orderData, proofFile) {
  try {
    const formData = new FormData();

    // Tambahkan semua field ke FormData
    formData.append(FORM_FIELDS.orderNumber, orderData.orderNumber);
    formData.append(FORM_FIELDS.nama, orderData.nama);
    formData.append(FORM_FIELDS.email, orderData.email);
    formData.append(FORM_FIELDS.noHp, orderData.noHp);
    formData.append(FORM_FIELDS.paket, orderData.paket);
    formData.append(FORM_FIELDS.harga, orderData.harga);
    formData.append(FORM_FIELDS.jenisDesain, orderData.jenisDesain);
    formData.append(FORM_FIELDS.deskripsi, orderData.deskripsi);
    formData.append(FORM_FIELDS.deadline, orderData.deadline);
    formData.append(FORM_FIELDS.metode, orderData.metode);

    // Tambahkan file bukti pembayaran jika ada
    if (proofFile) {
      formData.append(FORM_FIELDS.buktiPembayaran, proofFile);
    }
    ...
    console.error("Error mengirim data ke Google Form:", error);
    // Tetap lanjutkan proses meskipun gagal kirim ke Google Form
    return false;
  }
}

// Alternative: Submit via Apps Script (Lebih reliable)
async function submitViaAppsScript(orderData, proofFile) {
  try {
    // Konversi file ke base64 jika ada
    let proofBase64 = null;
    if (proofFile) {
      proofBase64 = await imageToBase64(proofFile);
    }
    ...
    const payload = {
      ...
      jenisDesain: orderData.jenisDesain,
      deskripsi: orderData.deskripsi,
      deadline: orderData.deadline,
      metode: orderData.metode,
      timestamp: new Date().toISOString(),
      buktiPembayaran: proofBase64,
      buktiFilename: proofFile ? proofFile.name : null,
    };
    ...
    console.log("Data berhasil dikirim via Apps Script");
    return true;
  } catch (error) {
    console.error("Error mengirim data via Apps Script:", error);
    return false;
  }
}
...
async function confirmPayment() {
  ...
  try {
    ...
    // Method 1: Direct ke Google Form (lebih sederhana tapi kurang reliable untuk file)
    //await submitToGoogleForm(orderData, paymentProofFile);

    // Method 2: Via Apps Script (lebih reliable, bisa handle file dengan base64)
    await submitViaAppsScript(orderData, paymentProofFile);

    console.log("Data berhasil dikirim ke Google Form");
  } catch (error) {
    console.error("Gagal mengirim ke Google Form:", error);
    // Tetap lanjutkan proses meskipun gagal
  }
...
function sendToWhatsAppWithProof() {
  ...
  let message =
    ...
    "%0A%0A" +
    "_Data juga telah tersimpan di Google Form_";
